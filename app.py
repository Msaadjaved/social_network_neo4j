...
class Database:
    ...
    # REMOVE all the sqlite CREATE TABLE statements and replace with:
    def _init_db(self):
    ...
    def create_user(self, username: str, name: str) -> int:
        with self._driver.session() as session:
            result = session.run("""
                MATCH (u:User)
                WITH count(u) AS cnt
                CREATE (new:User {id: cnt + 1, username: $username, name: $name})
                RETURN new.id AS id
    ...
    def get_feed(self, user_id: int) -> List[dict]:
        with self._driver.session() as session:
            result = session.run("""
                MATCH (me:User {id: $user_id})-[:FOLLOWS]->(author:User)-[:POSTED]->(p:Post)
                RETURN p.id AS id,
                    p.content AS content,
                    toString(p.timestamp) AS timestamp,
